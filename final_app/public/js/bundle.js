...
// Use maps (alternative to google maps for free, create account and use in client side)
// alternative to mapbox =  leaflet.js
// disable eslint for a js file:

/* eslint-disable */
console.log('mapbox code reached!!'); // we define mapbox code  into function so it is used by index.js script:

var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiZ2UwMHJnZSIsImEiOiJja3UwMWFleXcxdHRnMm5wODE5dGhxNTdyIn0.FAqmK-pmZQdWbTt95M70ow';
  var map = new mapboxgl.Map({
    // will place the map on an element with id of map:
    container: 'map',
  ...
  locations.forEach(function (loc) {
    // 1. Create marker
    var el = document.createElement('div'); // marker = defined in the style.css file

    el.className = 'marker'; // 2. Add marker

    new mapboxgl.Marker({
      element: el,
      // state that is the bottom of the pin where is going to be located the gps location:
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // 3. Add popup

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // 4. Extend map bounds to include current location
...
var showAlert = function showAlert(type, msg) {
  // hide alerts already exist:
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\">").concat(msg, "</div>");
  document.querySelector('body').insertAdjacentHTML('afterbegin', markup); // hide all alerts after 5 seconds:
...
// we export differently from common nodejs exports (CommonJS)
// ES6 modules use the export import syntax:
        ...
        switch (_context.prev = _context.next) {
          ...
          case 4:
            ...
            if (res.data.status === 'success') {
              (0, _alerts.showAlert)('success', 'logged in successfuly!'); // redirects to login page:
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            ...
            // need to reload the page which would then send the invalid cookie basically
            // to the server, so that one that we just received without a token
            // and then we are no longer logged in,
            if (res.data.status = 'success') {
              // to ensure we get a fresh page from the server
              location.reload();
            }
...
// type is either pwd or data:
// use single function to update data or pwd:
...
// we are going to use the GET checkout-session endpoint to get the checkout session to the client side:
  ...
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(tourId) {
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            // we paste the publishable api key from dashboard.stripe.com. We define it here because otherwise will appear in the global scope
            // were there is not the stripe library.
            stripe = Stripe('pk_test_51JrnDoBgQWuaBm9DHuBKX11D0Q0WQHphMAIYbrXkoblELjDnujEzUpyac6WIcV9hN4yfYfnJDtSjj3jXYWFVhD2600uVLyZxBy');
            _context.prev = 1;
            _context.next = 4;
          ...
          case 4:
            ...
            console.log(session); // 2. we use stripe object to create checkout form + charge the credit card
...
// we need to check if the DOM element exists in the doc (like map) before we do the JSON.parse:
// DOM ELEMENTS:
var mapbox = document.getElementById('map');
var loginForm = document.querySelector('.form--login');
var logoutBtn = document.querySelector('.nav__el--logout');
var userDataForm = document.querySelector('.form-user-data');
var userPasswordForm = document.querySelector('.form-user-password'); // we connect the button of bookings inside tour endpoint with the stripe.js file
...
} // were we are sending data to be updated on the server:


if (userDataForm) {
  userDataForm.addEventListener('submit', function (e) {
    e.preventDefault(); // here we need to programmatically recreate a multi-part form data to allow users update photo:
    ...
    var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(e) {
      ...
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              e.preventDefault(); // we will give feedback to user that we are saving the new updated pwd:
            ...
            case 7:
              // After the API call was successful, we should then go ahead and delete
...
if (bookBtn) {
  bookBtn.addEventListener('click', function (e) {
    e.target.textContent = 'Processing...'; // we change the text on the button

    var tourId = e.target.dataset.tourId; // which is the click of the book-tour button (found in tour.pug)
    // const {tourId} = e.target.dataset (with destructuring)
